...
export const firebaseService = {
  ...
  async addBatch(batch: Omit<Batch, 'id'>) {
    try {
      // Encode sensitive content data before storing
      const securedBatch = {
        ...batch,
        subjects: batch.subjects?.map(subject => ({
          ...
          sections: subject.sections?.map(section => ({
            ...section,
            contents: section.contents?.map(content => ContentSecurity.secureForStorage(content))
          }))
        }))
      };
  ...
  async getBatches() {
    try {
      ...
      // Decode sensitive content data after retrieving
      return batches.map(batch => ({
        ...
        subjects: batch.subjects?.map(subject => ({
          ...
          sections: subject.sections?.map(section => ({
            ...section,
            contents: section.contents?.map(content => ContentSecurity.prepareForDisplay(content))
          }))
        }))
      }));
    } catch (error) {
  ...
  async updateBatch(id: string, updates: Partial<Batch>) {
    try {
      // Encode sensitive content data before updating
      const securedUpdates = {
        ...updates,
        subjects: updates.subjects?.map(subject => ({
          ...subject,
          sections: subject.sections?.map(section => ({
            ...section,
            contents: section.contents?.map(content => ContentSecurity.secureForStorage(content))
      ...
      await updateDoc(doc(db, 'batches', id), securedUpdates);
    } catch (error) {
  ...
  onBatchesChange(
    ...
    return onSnapshot(q, 
      (snapshot) => {
        const rawBatches = snapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data()
        })) as Batch[];
        
        // Decode sensitive content data for real-time updates
        const decodedBatches = rawBatches.map(batch => ({
          ...batch,
          subjects: batch.subjects?.map(subject => ({
            ...subject,
            sections: subject.sections?.map(section => ({
