...
interface EnhancedVideoPlayerProps {
  ...
  userId?: string;
  username?: string;
  // Optional client-side obfuscation key/iv (base64url or base64). If provided, SW will decrypt segments with AES-CTR.
  obfuscationKeyB64?: string;
  obfuscationIvB64?: string;
  obfuscationCounterLength?: number; // defaults to 64
}
...
const EnhancedVideoPlayer: React.FC<EnhancedVideoPlayerProps> = ({
  ...
  useEffect(() => {
    ...
    const initializePlayer = async () => {
      ...
      try {
        if (isM3U8Url(src)) {
          // Try to route via Service Worker virtual manifest to hide raw URL on the element
          let effectiveSrc = src;
          try {
            const token = Math.random().toString(36).slice(2);
            // Map token -> real manifest in SW
            if (navigator.serviceWorker && navigator.serviceWorker.controller) {
              const msg: any = { type: 'MAP_TOKEN', token, manifestUrl: src };
              if (obfuscationKeyB64 && obfuscationIvB64) {
                msg.decrypt = {
                  algo: 'AES-CTR',
                  keyB64: obfuscationKeyB64,
                  ivB64: obfuscationIvB64,
                  counterLength: obfuscationCounterLength || 64
                };
              }
              navigator.serviceWorker.controller.postMessage(msg);
              effectiveSrc = `/v/manifest/${token}`;
            }
          } catch (_) {}
          
          // Additional URL obfuscation
          const obfuscatedSrc = effectiveSrc + '?t=' + Date.now() + '&r=' + Math.random().toString(36).substring(2);
  ...
  return (
    <>
      ...
      <DoubtBox
        ...
        isVisible={showDoubt}
        onToggle={() => setShowDoubt(!showDoubt)}
      />
    </>
  );
};
